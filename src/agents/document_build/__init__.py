...
class DocumentBuildAgent(BaseAgent):
	...
	def analyze(self, query: str, project_id: str, dataset_hint: str | None = None) -> dict[str, Any]:
		...
		return {
			...
			"table_name": final_state.get("table_name") or "",
			"table_path": final_state.get("table_path") or "",
			"mermaid_diagram": final_state.get("mermaid_diagram") or "",
			"sections": final_state.get("sections") or [],
			"data_dictionary": final_state.get("data_dictionary") or [],
			"typing_notes": final_state.get("typing_notes") or [],
			...
			"pending_technical": final_state.get("pending_technical") or [],
