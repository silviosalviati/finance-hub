...
function syncQAAnalyzeButtonState() {
  ...
  const blockedByValidation =
    ...
    qaDatasetValidationState.queryHash !== query;
...
function setQADatasetValidationStatus(kind, payload = {}) {
  ...
  const matchedTables = Number(payload.matchedTables ?? NaN);
  ...
  if (statusMetaEl && kind === "ok") {
    ...
    if (!Number.isNaN(matchedTables)) {
      chips.push(`<span class="qb-dataset-chip">🔎 ${matchedTables} usadas</span>`);
    }
  ...
  if (statusMetaEl && kind === "error") {
    statusMetaEl.innerHTML =
      '<span class="qb-dataset-chip">⚠️ Revise a query e as tabelas referenciadas</span>';
  }
...
async function validateQAQueryContext() {
  ...
  setQADatasetValidationStatus("checking", {
    ...
    message: "Identificando dataset e tabelas no BigQuery/Data Catalog...",
  });
  ...
  try {
    ...
    if (payload.valid) {
      ...
      qaDatasetValidationState.queryHash = querySnapshot;
      setQADatasetValidationStatus("ok", {
        ...
        matchedTables: Array.isArray(payload.matched_tables)
      ...
      setQADatasetValidationStatus("error", {
        ...
        message:
          payload.message ||
          "Nao foi possivel validar dataset e tabelas para o Query Analyzer.",
      });
    }
  } catch (err) {
    qaDatasetValidationState.status = "invalid";
...
async function runAnalyze() {
  ...
  const dataset_hint = document.getElementById("qa-dataset")?.value.trim() || "";
  ...
  if (!project_id) {
    showQAError("Cole a query para detectar e validar o Project ID automaticamente.");
    return;
  }

  if (!dataset_hint) {
    showQAError("Cole a query para detectar e validar o Dataset hint obrigatorio.");
    return;
  }
  ...
  if (!isValidContext) {
    showQAError(
      "Valide dataset e tabelas da query no BigQuery/Data Catalog antes de analisar.",
    );
    return;
  }
